...
// ── LOGIN ─────────────────────────────────────────────────────────────────────
// ── MFA GATE — verify 2FA for cached admin sessions ─────────────────────────
const MFAGate=({onVerified})=>{
  ...
  useEffect(()=>{
    (async()=>{
      try{
        ...
        if(error||!data){onVerified();return;}
        const totp=(data.totp||[]).filter(f=>f.status==="verified");
        if(totp.length===0){onVerified();return;}
        setFactorId(totp[0].id);
        setStage("verify");
      }catch(e){
        onVerified(); // fail open — don't lock out
      }
    })();
  },[]);
  ...
  const verify=async()=>{
    ...
    try{
      ...
      if(ce)throw ce;
      const{error:ve}=await supabase.auth.mfa.verify({factorId,challengeId:ch.id,code:otp.trim()});
      if(ve)throw ve;
      onVerified();
      ...
      <div style={{width:"100%",maxWidth:380}}>
        <div className="card" style={{padding:26}}>
          <div style={{textAlign:"center",marginBottom:20}}>
            ...
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:18,color:"#212121"}}>VERIFY YOUR IDENTITY</div>
          ...
          <button onClick={verify} className="btn ba" style={{width:"100%",justifyContent:"center",padding:"11px",marginTop:12}} disabled={loading||otp.length<6}>
  ...
  const go=async e=>{
    ...
    try{
      ...
      if(verified.length>0){
        ...
        // No MFA yet — enroll
        const{data:enroll,error:ee}=await supabase.auth.mfa.enroll({factorType:"totp"});
        if(ee)throw ee;
        setFactorId(enroll.id);
        setSecret(enroll.totp.secret);
        setOtpauth(enroll.totp.qr_code);
        setStage("enroll");
      }
    }catch(e){
      setErr(e.message);
